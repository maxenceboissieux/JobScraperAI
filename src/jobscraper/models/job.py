...
class JobOffer(BaseModel):
    ...
    experience_level: Optional[ExperienceLevel] = Field(
        None, description="Niveau d'expérience requis"
    )
    ...
    scraped_at: datetime = Field(
        default_factory=datetime.now, description="Date de scraping"
    )
...
class SearchCriteria(BaseModel):
    ...
    keywords: List[str] = Field(
        default_factory=list, description="Mots-clés de recherche"
    )
    ...
    radius_km: Optional[int] = Field(
        None, description="Rayon de recherche en km (5, 10, 25, 50, 100)"
    )

    # Filtres de contrat et expérience
    contract_types: List[ContractType] = Field(
        default_factory=list, description="Types de contrat"
    )
    experience_levels: List[ExperienceLevel] = Field(
        default_factory=list, description="Niveaux d'expérience"
    )

    # Lieu de travail
    workplace_types: List[WorkplaceType] = Field(
        default_factory=list, description="Types de lieu de travail"
    )
    ...
    companies: List[str] = Field(
        default_factory=list, description="Entreprises spécifiques"
    )
    exclude_companies: List[str] = Field(
        default_factory=list, description="Entreprises à exclure"
    )
    ...
    posted_within_days: Optional[int] = Field(
        None, description="[Deprecated] Utiliser date_posted"
    )
